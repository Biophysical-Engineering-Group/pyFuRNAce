...
def primers_tab(seq):
    ...
    ###
    with st.expander("Melting temperature parameters"):
    ...
    with col1:
        st.write("Coding primer:")
        c_bases = st.slider('N° bases:', min_value=1, max_value=50, value = 21, key="coding_primer")
        c_primer = seq[:c_bases]
        write_format_text(c_primer)
        mts[0] = round(mt_correct(c_primer),2)
        st.write(f"%GC content: {gc_fraction(c_primer, ambiguous='ignore') * 100}; Tm: {mts[0] }")
    # settings for the non-coding primer
    with col2:
        st.write("Non-coding primer:")
        nc_bases = st.slider('N° bases:', min_value=1, max_value=50, value = 21, key="non_coding_primer")
        nc_primer = seq[-nc_bases:].reverse_complement()
        write_format_text(nc_primer)
        mts[1] = round(mt_correct(nc_primer),2)
        st.write(f"%GC content: {gc_fraction(nc_primer, ambiguous='ignore') * 100}; Tm: {mts[1]}")
