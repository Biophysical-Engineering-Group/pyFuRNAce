...
funny_bootstrap_icons = ['robot', 'trash', 'umbrella', 'camera', 'cart', 'cpu', 
                         'cup-straw', 'trophy', 'palette', 'cup-straw', 
                         'camera-reels', 'puzzle', 'hourglass-split', 'mortarboard']
...
def origami_general_options(origami, expanded=True):
    with st.expander('General settings', expanded=expanded):
        ...
        ### select alignment
        with cols[0]:
            align = st.radio("Alignment type:", 
                             ['To the left', 
                             'Junctions alignment: first', 
                             'Line center'], 
                             index=0, 
                             key='alignment')
...
def simple_origami():
    with st.form(key='simple_origami_form'):
        dt_text = st.text_input("Enter a list of the angles between helices, "
                                "separated by commas", 
                                "120, ", 
                                key='dt_text_list',
        ...
        main_stem_default = 11 * ((max([abs(dt) for dt in dt_list], 
                                        default=0) + 17) // 11 + 1)
        col1, col2 = st.columns(2, vertical_alignment='bottom')
        with col1:
            kl_columns = st.number_input('Kissing loop columns:', 
                                         min_value=1, 
                                         value=1, 
                                         help='number of KL repeats in the helix')
        with col2:
            main_stem = st.number_input('Spacing between crossovers (bp):', 
                                        min_value=22, 
                                        value=main_stem_default, 
                                        step=11, 
                                        help='The length of the consecutive stems '
                                              'in the helix')

        submitted = st.form_submit_button("Submit")
        if submitted:
            st_state.origami = pf.simple_origami(dt_list=angle_list, 
                                                         kl_columns=kl_columns, 
                                                         main_stem=main_stem, 
                                                         add_terminal_helix=True, 
                                                         align=st_state.origami.align,
                                                         use_angles=True)
            st_state.code.append(f'origami = pf.simple_origami(dt_list={angle_list}, '
...
def initiate_session_state():
    ...
    if 'custom_motifs' not in st_state:
        st_state.custom_motifs = [('Custom Motif', 
                                   'dpad', 
                                   pf.Motif(lock_coords=False))]
...
def make_motif_menu(origami):
    ...
    st_state.max_pk_index = max([abs(int(x.pk_index.replace("'", ""))) 
                                        for line in pk_motifs 
                                            for x in line], 
                                default=0) + 1
    ...
    if selected_motif == 'Connections':
        ...
        with col1:
            motif_selected = option_menu(None,
                                        ...
                                        icons=[f"bi bi-{l[1]}" 
                                               for l in st_state.custom_motifs],
                                        menu_icon="cast", 
                                        orientation="horizontal",
                                        styles=second_menu_style)
        with col2:
            new_name = st.text_input(":green[Add custom motif with name:]", 
            ...
            if new_name:
                ...
                st_state.custom_motifs.append((new_name, 
                                               icon, 
                                               pf.Motif(lock_coords=False)))
                st_state.custom_key += 1
                st.rerun()

        current_custom_motif = [m for m in st_state.custom_motifs 
                                        if m[0] == motif_selected][0][2]
    ...
    if motif_add:
        st.markdown("<hr style='margin-top:-0em;margin-bottom:-1em' />", 
...
def select_line(f_col1=None, f_subcol2=None, f_subcol3=None):
    ...
    with col2:
        ...
        line_index = st.number_input("**Select line index:**", 
                                    min_value=-1,
        ...
        with subcol2:
            ...
            if len(st_state.origami) > 1 or st_state.origami[0]: 
                ...
                if len(st_state.origami) > 0:
                    del_line = st.button("Delete line", 
                                         key = 'del_line', 
                                         help="Delete the choosen line")
                    if del_line:
                        st_state.origami.pop(st_state.line_index) #remove choosen helix
    ...
    with col3:
        ...
        if motif_index > max_val:
            motif_index = max_val
        motif_index = st.number_input("**Select motif index:**", 
                                      min_value=0, 
                                      max_value=max_val, 
                                      value=motif_index, 
    ...
    with col4:
        if st.button("Duplicate line", key='duplicate_line'):
            ...
            st_state.code.append(f'origami.duplicate_line({line_index}, '
    ...
    with col5:
        ...
        with subcol2:
            ...
            if paste_button:
                ...
                st_state.code.append(st_state.copied_motif_text + 
...
def add_motif(origami):
    ...
    # If the motif is a generic motif, generate the motif text buffer
        ...
        for s in st_state["custom_strands"]:
            st_state.motif_buffer += (f"strands.append(pf.Strand('{s}', "
        ...
        st_state.motif_buffer += (f"motif = pf.Motif(strands, "
    ...
    def f_subcol2(linex_index):
        ...
        if len(st_state.origami) > 1 or st_state.origami[0]: 
            add_line = st.button("Add line", 
                                 key='Insert_line', 
                                 help=f"Add a new line n° {linex_index}.")
            if add_line:
...
def copy_motif(key='', motif=None, motif_slice=None):
    ...
    if copy_button:
        if not motif:
            ...
            st_state.copied_motif_text = (f"motif = origami[{motif_slice}].copy() "
    ...
    for y in range(y_size):
        for x in range(x_size):
            if (x, y) in s_pos_to_sym:
                content += (f'<a href="javascript:void(0);" id="{x},{y}" '
...
def custom_text_input(current_custom_motif):
    ...
    if current_custom_motif:
        ...
        motif_list = [[' '] * add_char + [char for char in line] + [' '] 
    ...
    for s in current_custom_motif:
        ...
        if (s.directionality == '53' 
            and motif_list[ys][xs] == ' '): 
            motif_list[ys][xs] = '5'
         
        # add 5' to the end of the strand
        elif (s.directionality == '35' 
              and motif_list[ye][xe] == ' '):
            motif_list[ye][xe] = '5'

    current_custom_motif_str = '\n'.join(''.join(line) for line in motif_list)

    strand_text = st.text_area("Motif text: draw a motif where each Strand starts "
                               "with a 5", 
    ...
    if strand_text != current_custom_motif_str:
        if '5' not in strand_text:
            ...
            current_custom_motif.replace_all_strands(new_motif._strands, 
                                                     copy=False)
...
def structure_converter(current_custom_motif):
    ...
    with col1:
        structure = st.text_input("Dot-bracket structure:", 
                                 ...
                                 help="Add a dot-bracket structure to convert it into "
                                      "a 3D motif.")
    with col2:
...
def upload_3d_interface(strand, strand_num, current_custom_motif):
    file_3d = st.file_uploader(f"3D coordinates (OxDNA format) of "
                               f"**strand {strand_num}**", 
                               type=['dat', 'pdb'],
    ...
    dummy_help = ('The dummy base is a base in the coordinates that is not '
                  'part of the sequence, but it is used to connect other strands '
                  'to the beginning or end of the strand.')

    with dummy_cols[0]:
        dummy_start = st.toggle('Start dummy nucleotide', 
                                key=f'dummy_start_custom', 
                                help=dummy_help)
    with dummy_cols[1]:
        dummy_end = st.toggle('End dummy nucleotide', 
                              key=f'dummy_end_custom', 
                              help=dummy_help)
...
def update_strand(strand, strand_num, current_custom_motif, coords=None):
    ...
    current_custom_motif.replace_all_strands(st_state["custom_strands"], 
                                             copy=False)
    if coords:
        st.success("3D coordinates uploaded successfully!", 
...
def strand_number_button(current_custom_motif, delete=True):
    ...
    strand_num = st.radio("Select strand:", 
                          list(range(len(strands))), 
                          index=len(strands)-1)
...
def custom(current_custom_motif):
    ...
    with col2:
        try:
            clicked = click_detector(generate_custom_motif_text(strand, 
                                                    x_size=x_dots, 
                                                    y_size=y_dots), 
    ...
    def move_strand(clicked, add_nucl=None):
        ...
        ### if the strand has only one position, add the strand direction
            ...
            ### check the direction and in case add the new position
                ...
                normal_direction = tuple([i // abs(i) if i != 0 else 0 
                                                            for i in direction])
                direction_difference = (normal_direction[0] - strand.end_direction[0],
                                        normal_direction[1] - strand.end_direction[1])
                distance = max(abs(pos[0] - strand.end[0]), 
                               abs(pos[1] - strand.end[1]))
    ...
    with col1:
        start_x = st.number_input("Start x:", 
                                  min_value=0, 
                                  value=strand.start[0], 
                                  key=f'start_x_custom')
    with col2:
        start_y = st.number_input("Start y:", 
                                  min_value=0, 
                                  value=strand.start[1], 
                                  key=f'start_y_custom')
    with col3:
        strand_direction_ind = [d for d in pf.Direction].index(strand.direction)
        new_dir = st.selectbox('Start direction:', 
                               direction_list, 
                               index=strand_direction_ind, 
                               key=f'dir_custom')
        new_dir_tuple = pf.Direction[new_dir]
    with col4:
        seq_dir = st.selectbox('Directionality:', 
                               ['35', '53'], 
                               index=['35', '53'].index(strand.directionality), 
                               key=f'seq_dir_custom')
    with col5:
        new_strand = st.text_input(f'New strand (strand directionality: '
                                   f'{strand.directionality}) ', 
                                   value=str(strand), key=f'strand_custom')
    ...
    new_db = st.text_input('Add dot-bracket notation:', 
...
def update_code(code_text):
    ...
    for kw in forbidden_keywords:
        if kw in code_text:
            st.error(f"Forbidden keyword detected in the code: '{kw}'", 
...
def code():
    ...
    if (response_dict['id'] != st_state["last_code_id"] 
            and (response_dict['type'] == "submit" 
                 or response_dict['type'] == "selection") ):
            ...
            if score > 30:
                if st.button('Try to fix the folding pathway', key='fix_pathway',
                    ...
                    with st.spinner("Optimizing the folding pathway..."):
                        ...
                        code_lines += ['\norigami = origami.improve_''folding_pathway'
                                       '(kl_delay=150)\n']
                        update_code('\n'.join(code_lines))
...
def build_origami_content(barriers=None):
    ...
    if st_state.gradient:
        ...
        c_map = [mcolors.to_hex(cmap(i)) 
                            for i in np.linspace(0, 1, tot_len + oxview_offset)]
    ...
    content = ("<div style='font-family: monospace; white-space: nowrap; "
               "overflow-x: auto;'>")
    
    # add a column with the line number
    content += (f'<div style="display:inline-block; font-family: monospace;'
    ...
    for y, line in enumerate(origami_list):
        ...
        elif (isinstance(next_line_nr, int) 
        ...
        for x, char in enumerate(line):
            ...
            if char == ' ':
                ...
                content += (span_text 
                            + f'color: {highlight_color}; line-height:1;">5</span>')
            elif char == '2':
                content += (span_text 
                            + f'color: {highlight_color}; line-height:1;">3</span>')
                
            # do not highlight the base pair in red
            elif char in pf.bp_symbols:
                content += (span_text 
                            + f'color: {color}; line-height:1;">{char}</span>' )
                ...
                content += (f'<a style="font-family: monospace; color: {color}; '
        ...
        if motif_slice and motif_slice[0] == st_state.line_index:
            if len(origami[motif_slice[0]]) == st_state.motif_index:
                content += (span_text 
                            + f'color: {highlight_color}; line-height:1;">│</span>')
        content += '<br />'
...
def display_origami(barriers=None):
    ...
    try:
        ...
        clicked = click_detector(content, 
                                 key=f"origami_click{st_state.ori_click_count}")
    except pf.MotifStructureError as e:
...
def clicked_options(clicked):
    ...
    if clicked:
        ...
        if (st_state.line_index != motif_slice[0] 
        ...
        with col1:
            st.markdown(f'Selected {nucl_text} :orange[{motif_class_name}]: '
...
def display_structure_sequence():
    ...
    if origami.structure:
        ...
        if motif_slice in origami.index_shift_map:
            ...
            for pos in motif.seq_positions:
                indexes.append(origami.seq_positions.index((pos[0] + shifts_x, 
                                                            pos[1] + shift_y)))

        for i in range(len(structure_list)):
            if i in indexes:  # highlight the selected motif
                structure_list[i] = (f'<span style="color: #D52919; line-height:1;">'
                                    f'{html.escape(structure_list[i])}</span>')
                sequence_list[i] = (f'<span style="color: #D52919; line-height:1;">'
        ...
        st.markdown(f'**Structure length: '
                    f':green[{str(len(origami.structure.replace("&", "")))}]**')
        
        scrollable_text(f">Origami</br>{''.join(sequence_list)}</br>"
...
def edit(x, y):
    ...
    if st_state.modified_motif_text:
        st_state.code.append(f'motif = origami[({motif_slice[0]}, '
                             f'{motif_slice[1]})] '
        ...
        with st.popover('Advanced feature, modify the strands of the selected motif:',
            ...
            for i, s in enumerate(motif_copy):
                ...
                with col5:
                    new_strand = st.text_input(f'New strand (strand directionality: '
                                               f'{s.directionality}) ', 
                                               value=str(s), key=f'strand_{x}_{y}_{i}')
            ...
            try:
                ...
                if not st_state.modified_motif_text and not shifted:
                    st.write(':green[Structure is updated]') 
                else:
                    st.warning('Be careful before updating, there is a high risk of '
                    ...
                    if update:
                        ...
                        st_state.code.append(f'motif = origami[({motif_slice[0]}, '
                                             ...
                                             f'at index, line: {motif_slice}' 
                                             + st_state.modified_motif_text)
                        
                        st_state.origami[motif_slice] = motif_copy
                        st.rerun()
