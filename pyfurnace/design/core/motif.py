...
class Motif(Callback):
    ...
    *args : list of Strand
    ...
    lock_coords : bool, default True
        ...
        When a strand coordinate transformed, all other strands are transformed accordingly.
    ...
    **kwargs : dict
    ...
    autopairing : bool
        Indicates whether the motif automatically pairs bases based on complementary interactions.
    junctions : dict
        A dictionary with 2D Directions as keys, and the positions of the open ends as values.
    map : dict
    ...
    sequence_index_map : dict
        A mapping of 2D positions of the bases in the motif to their respective sequence indices.
    pair_map : dict
    ...
    def __init__(self, 
        ...
        *args : list of Strand
        ...
        if not all(isinstance(s, Strand) for s in all_strands):
            raise ValueError("All the elements in the strands input must be of type Strand."
                             f"Got types {[type(s) for s in all_strands]}.")
        ...
        ### LOCK COORDINATES
        if self.lock_coords:
            # IMPORTANT: look at the _coords to avoid locking strands that don't have coordinates 
    ...
    def align(motifs: List['Motif'], 
        ...
        *args : List[Motif]
        ...
        if align_to not in ("first", "last", "center"):
            raise ValueError(f"{align_to} is not a valid value for the align_to parameter."
                             ' The align_to parameter must be "first", "last" or "center"')
    ...
    def __str__(self) -> str:
        ...
        for pos1, pos2 in self.basepair.items():
            ...
            if pos[1] < len(canvas_repr) and pos[0] < len(canvas_repr[0]) and canvas_repr[pos[1]][pos[0]] == ' ': 
    ...
    def __add__(self, other: 'Motif') -> 'Motif':
        ...
        x_shifts = self.get_sequential_shift([self_copy, other_copy], position_based=False)
    ...
    def __iadd__(self, other: 'Motif') -> 'Motif':
        ...
        if self:
            ...
            x_shifts = self.get_sequential_shift([self, other_copy], position_based=False)
            # move motif to the right of the first motif
            other_copy.shift((x_shifts[1], 0)) 
    ...
    def base_map(self) -> Dict[Tuple[int, int], int]:
        """A dictionary with the position occupied by the bases as key and the strand index as value."""
        if self._base_map:
    ...
    def sequence_index_map(self) -> Dict[Tuple[int, int], int]:
        """A dictionary with the position occupied by the bases as key and the index of the sequence as value."""
    ...
    def junctions(self) -> Dict[Tuple[int, int], List[Tuple[int, int]]]:
        '''
        A dictionary with the four junctions directions as key (right: (1,0), bottom: (0,1), left: (-1,0), top: (0,1)) 
        and an ordered list of junction positon as value.
        '''
    ...
    def basepair(self) -> BasePair:
        """A dictionary with positions as keys and the paired positions as values."""
    ...
    @basepair.setter
    def basepair(self, basepair_dict: Union[Dict[Position, Position], BasePair]) -> None:
    ...
    def structure(self, structure: str) -> None:
        """Set the dot bracket notation of the motif, it updates the basepair dictionary """
    ...
    def sequence(self, seq_list: str = None) -> None:
        ...
        seq_list : str or Sequence or list of str or Sequence
            The list of sequences to set to the strands. If a single string is passed, it is split by '&'.
    ...
    def lock_coords(self, lock_coords):
        """Set wether the coordinates of the motif must be locked in the same coordinate block."""
            ...
            for pos in junc_vert:
                if pos in s.map:
                    extend_strand.append(Strand('│' * abs(shift[1]), 
                                                start=(pos[0] + shift[0], pos[1]), 
                                                direction=(0, int(copysign(1, shift[1])))))
            for pos in junc_hor:
                if pos in s.map: 
                    extend_strand.append(Strand('─' * abs(shift[0]), 
                                                start=(pos[0], pos[1] + shift[1]), 
                                                direction=(int(copysign(1, shift[0])), 0)))
    ...
    def _updated_basepair(self, **kwargs) -> None:
        ...
        **kwargs
        ...
        Notes
        -----
        This function resets the cached dot-bracket notation representation of the motif.
        """
    ...
    def _check_addition(self, 
                            other: "Motif", 
                            direction: Union[Position, Tuple[int, int]] = Direction.RIGHT) -> None:
        """
        ...
        if not junction1 or not junction2:
            raise MotifStructureError(f"The motifs cannot be added in the direction {direction}, the junctions are missing." 
                                        f" Junctions motif1: {junction1}, Junctions motif2: {junction2}." 
                                        " If you want to concatenate the motifs, use pf.Motif.concat() method.")
        elif len(junction1) != len(junction2):
            raise MotifStructureError(f"The number of connecting strands is different in the direction {direction}." 
                                        " Junctions motif1: {junction1}, Junctions motif2: {junction2}")
    ...
    def copy_strands_preserve_blocks(strands: List["Strand"], 
                                     motif: Optional["Motif"] = None) -> List["Strand"]:
        """
