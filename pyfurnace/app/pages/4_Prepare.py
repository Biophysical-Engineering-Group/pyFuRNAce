...
def calculate_annealing(seq, mts, c_primer, nc_primer, tm_kwargs):
    ...
    with st.expander("Dimerization preview"):
        ...
        col1, col2 = st.columns(2, gap='large')
        with col1:
            self_dimer1 = check_dimer(c_primer, 
                                      c_primer, 
                                      basepair={'A': 'T', 
                                                'T': 'A', 
                                                'C': 'G', 
        ...
        with col2:
            self_dimer2 = check_dimer(nc_primer, 
                                      nc_primer, 
                                      basepair={'A': 'T', 
                                                'T': 'A', 
                                                'C': 'G', 
        ...
        self_dimer12 = check_dimer(c_primer, 
                                   nc_primer, 
    ...
    t_anneal = annealing_temp(mts, 
                              seq, 
                              tm_kwargs, 
                              method=st.session_state.annealing_model.split()[0])
    if t_anneal < 50:
        st.warning('Is suggested a temperature of annealing higher than 50°C.', 
                   icon="⚠️")
        anneal_color = 'red'
...
def primers_setup():
    ...
    seq = sanitize_input(st.text_input("Input sequence:", 
                                       value = st.session_state["dna_template"]))

    # check the symbols in the sequence
    if set(seq) - symbols:
        st.warning('The sequence contains symbols not included in the '
...
def md_setup():
    ...
    col1, col2 = st.columns(2)
    with col1:
        oxdna_dir = st.text_input('OxDNA directory '
                                  '(e.g. "~/Documents/software/oxDNA")')
    with col2:
        temp = st.number_input('Temperature (°C)', 
                               value=37, min_value=0, max_value=100, step=1)
    ...
    zip_path = prep.oxdna_simulations(origami=st.session_state.origami,
