...
def forna_options(lenght):
    ...
    with col1:
        st_state.zoomable = st.checkbox("Zoomable", 
                                                value=True, 
                help = """Enable zooming in and out the structure""")
    with col2:
        st_state.animation = st.checkbox("Interact", 
                                                 value=False, 
                help = """Enable interaction with the structure""")
    with col3:
        st_state.node_label = st.checkbox("Label", 
                                                  value=True, 
    ...
    with col1:
        ...
        scheme_options = ["sequence", 
                          ...
                          "color range", 
                          "custom"]
        st_state.color_scheme = st.selectbox("Color scheme", 
                                                     scheme_options, 
                                                     index=1)

    with col2:
        st_state.height = st.slider("Frame height", 
                                            min_value=10, 
                                            max_value=800, 
                                            value=300, 
    ...
    with col3:
        st_state.label_interval = st.slider("Label every", 
                                                    min_value=0, 
...
def generate_sequence():
    ...
    if ('origami' in st_state 
        ...
        try:
            ...
            st.error(f"Error while loading the sequence into the origami"
                     f" design: {e}")
...
if __name__ == "__main__":
    ...
    # RNA INPUTS
    structure = st.text_input("RNA strucutre (dot-bracket notation)", 
                              value=st_state.generate_structure)
    sequence_constraint = st.text_input("Sequence constraints", 
                                        value=st_state.generate_sequence)
    pseudoknot_info = st.text_input("Pseudoknot constraints (semicolon-separated)", 
    ...
    with col1:
        use_forna = st.toggle("Forna display",
                              key='forna_display',
        ...
        try: 
            ...
            use_forna = False
            st.error("st-forna-component is not installed. "
    ...
    if sequence_constraint and sequence_constraint[0] != 'G':
        ...
        with col2:
            new_start = st.text_input("Start the sequence with", 
                                value='GGGA',
