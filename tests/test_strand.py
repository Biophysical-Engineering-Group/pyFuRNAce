...
def test_strand_go_through(strand):
    strand_structure, next_position = strand.go_through(strand.start)
    assert strand_structure == 'A\\U|G/C'
    assert next_position == (-1, 4)


def test_strand_is_at_start_or_end(strand):
    assert strand.isAtStartOrEnd((0, 0)) == 'start'
    assert strand.isAtStartOrEnd(strand.end) == 'end'
    assert strand.isAtStartOrEnd((1, 1)) is None
