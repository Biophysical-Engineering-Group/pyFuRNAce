...
#  name as key and function as value
tm_models = {"Nearest Neighbor": ['DNA_NN4', 
                                  'DNA_NN1', 
                                  'DNA_NN2', 
                                  'DNA_NN3', 
                                  'RNA_NN1', 
...
default_values = {"Na": 0, "K": 50, "Tris": 20, "Mg": 1.5,
                  "dNTPs": 0.2, "Method": 7, "DMSO (%)": 0,
...
def make_tm_calculator(method_name: str = "Nearest Neighbor",
                        model_name: str = "DNA_NN4",
                        primer_conc: float = 500,
                        tm_kwargs: dict = default_values.copy()) -> callable:
    """
    ...
    if tm_kwargs.get('DMSO', 0) > 0:
        return lambda seq: mt.chem_correction(partial_func(seq), 
                                              DMSO=tm_kwargs['DMSO'])
...
def annealing_temp(mts: List[float], 
                   seq: str, 
                   tm_kwargs: dict, 
                   method: str = 'IDT') -> float:
    """
    ...
    if method == 'IDT':
        ...
        if 'DMSO' in method_kwargs:
            method_kwargs.pop('DMSO')
        t_anneal = (0.3 * min(mts) 
                    + 0.7 * mt.chem_correction(mt.Tm_GC(seq, 
                                                        valueset=7, 
                                                        **method_kwargs), 
                                                DMSO=tm_kwargs['DMSO']) - 14.9)
    else:  # Phusion method
        t_anneal = min(mts)
...
def check_dimer(seq1: str, 
    ...
    for start_ind in range(len(seq1) + len(seq2) - 2):
        ...
        for ind, b in enumerate(bottom_seq):
            if (b.translate(basepair) == top_seq[ind] 
                    and b != ' ' and top_seq[ind] != ' '):
                bp += '┊'
            else:
                bp += ' '
        num_bp = bp.count('┊')
        ...
        if num_bp > max_match[1].count('┊'):
            max_match = [index_num 
                         + top_seq, ' ' * len(index_num) 
                         + bp, ' ' * len(index_num) + bottom_seq]
        if dict_format:
...
def auto_design_primers(seq: Seq, 
                        target_temp: float = 65.0, 
                        tm_func: Callable = make_tm_calculator()
